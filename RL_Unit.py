...
save_directory = 'C:\\Users\\svalizad\\Desktop\\Automated ML\\ADP_Journal Paper'
...
model_number, preprocess_number, train_strategy, transferLearning, augmentation = 12, 25, 2, 2, 2

options = model_number * preprocess_number * train_strategy * transferLearning * augmentation

Q_Values = np.random.uniform(0, 1, size=(Total_Episodes, options))
...
for episode in range(Total_Episodes):
    ...
    for iteration in range(Iterations):
        ...
        if random_event <= epsilon:
            ...
            print('Random Event taken!')
            
        elif random_event > epsilon:
            
            max_i_th_Q = np.max(Action_selections)
            
            index = np.where(max_i_th_Q == Action_selections)
